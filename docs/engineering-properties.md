...
documented KBP execution path.

## `CLM-0002` — complete current RunPassport binding

**Statement.** A current RunPassport binds authorization, custody outcome,
...
**Statement.** The standalone RunPassport verifier needs one passport file and
no network, repository checkout, running KBP service, private key, or
...
**Meaning.** Designed refusals are retained as first-class results rather than
relabeled as successful cargo runs. The aggregate shows both admitted and refused
